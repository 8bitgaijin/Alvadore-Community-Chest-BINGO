...
try:
    ...
    print(f"Required third-party module Pygame missing: {e}")  
...
native_w = info_object.current_w
native_h = info_object.current_h
logger.info(f"Detected display resolution: {native_w}x{native_h}")
...
# How long to review most recently called ball
REVIEW_DURATION = 3
...
def reset_board(state):
    ...
    Args:
        state (dict): The current game state containing 'balls'
        and 'drawn_balls'.
...
def restore_last_game(state):
    ...
    Args:
        state (dict): The current game state containing 'last_game' 
        and 'drawn_balls'.
    ...
    """
    try:
        # Safely check if 'last_game' exists and is not empty
...
def draw_bingo_pattern(state):
    ...
    if pattern_image:
        image_rect = pattern_image.get_rect(center=(pattern_value_x + 200, 
                                                    pattern_value_y + 300))
        screen.blit(pattern_image, image_rect)
    else:
        render_text_with_shadow(
            BINGO_PATTERNS[current_pattern], 
            PREVIOUS_FONT, 
            FONT_COLOR, 
            QUESTION_COLOR,
            pattern_value_x, 
...
def render_text_with_shadow(text, 
                            font, 
                            color, 
                            shadow_color, 
                            x, 
                            y, 
                            shadow_offset=DROP_SHADOW_OFFSET):
    """
    ...
    try:
        ...
        shadow_rect = shadow_surface.get_rect(topleft=(x + shadow_offset, 
                                                       y + shadow_offset))
...
def calculate_animation_frame(
    ...
    """
    Calculate the current position, radius, 
    and font size based on the progress (t).
    ...
    current_font_size = int(start_font_size + t * 
...
def animate_ball_transition(
    ...
    """
    Animates the ball and text transition between announcement and 
    review states, with particle effects.
    ...
    while current_frame < total_frames:
        ...
        spawn_particles_for_frame(current_frame, 
                                  particles, 
                                  current_x, 
                                  current_y)
...
def render_bingo_letters(font_color):
    ...
    for row_number, letter in enumerate(BINGO_RANGES.keys()):
        ...
        shadow_rect = shadow_surface.get_rect(
            center=(left_margin + DROP_SHADOW_OFFSET, 
                    letter_y + DROP_SHADOW_OFFSET)
        )
...
def render_bingo_pattern(state, font_color):
    ...
    shadow_surface = PREVIOUS_FONT.render(pattern_message, 
                                          True, 
                                          QUESTION_COLOR)
    shadow_rect = shadow_surface.get_rect(
        center=(WIDTH // 2 + DROP_SHADOW_OFFSET, 
                HEIGHT - 50 + DROP_SHADOW_OFFSET)
    )
...
def display_ball_review(state):
    """
    Displays the newest ball large on the right and 
    up to five previous balls stacked on the left.
    """
    clear_screen()
...
def display_confirmation(message):
    ...
    shadow_rect = shadow_surface.get_rect(center=(WIDTH // 2 + DROP_SHADOW_OFFSET, 
                                                  HEIGHT * 0.25 + DROP_SHADOW_OFFSET))
    screen.blit(shadow_surface, shadow_rect)
...
def handle_confirmation(state, key, action, flag_key):
    ...
    Args:
        ...
        action (callable): The function to call if the 
        confirmation key is pressed.
